...
class_net=Cla_net().to(device)
class_net.load_state_dict(torch.load(load_model_path))

logger.critical("class_net have {} paramerters in total".format(
    sum(x.numel() for x in class_net.parameters())
    ))
...
#加载优化器，损失函数
optimizer=torch.optim.Adam(class_net.parameters(),lr=lr)
...
for i in range(epochs):
    ...
    torch.save(class_net.state_dict(),'result/model/cla_net_{}.pth'.format(i))
    #验证
    class_net.eval()
    with torch.no_grad():
