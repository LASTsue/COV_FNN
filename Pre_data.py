...
class Z_Data(data.Dataset):
    def __init__(self,data_type,img_type=0) -> None:
        ...
        if img_type==0:
            ...
            exit()
        self.path='datasets/COVID-19/'
        if data_type=='train':
            self.data=pd.read_csv('datasets/COVID-19/train.csv')
        elif data_type=='val':
            self.data=pd.read_csv('datasets/COVID-19/val.csv')
        elif data_type=='test':
            self.data=pd.read_csv('datasets/COVID-19/test.csv')
        else:
